...
def sugarcane_calc(ndvi_fp, ndvi_nrs, max_yield, pct_n, dry_matter, nue):
    """
    ...
    Parameters:
      ...
      dry_matter — fraction of the stalk that is dry matter (default 0.30 = 30%)
                   sugarcane stalks are 70-75% water; nitrogen is only in the dry portion
    ...
    # × dry_matter removes the water fraction — N is only in the dry matter
    # × (pct_n / 100) converts the percentage to a decimal
    # Formula: GNUP = yield × 2000 lbs/ton × dry_matter × (%N / 100)
    gnup_yp0 = yp0 * 2000 * dry_matter * (pct_n / 100)
    ...
    gnup_ypn = ypn * 2000 * dry_matter * (pct_n / 100)
...
def rice_calc(ndvi_fp, ndvi_nrs, days, max_yield, pct_n, nue):
    ...
    The key difference from sugarcane: rice uses INSEY instead of raw NDVI.
    INSEY = NDVI / days from planting — normalizes for plant age, because
    older rice plants have higher NDVI even without extra N.

    Parameters:
      ndvi_fp    — NDVI of the farmer's practice field
      ndvi_nrs   — NDVI of the N-Rich reference strip
      days       — days from planting date to NDVI sensing date
    ...
    # Step 1: INSEY — NDVI normalized by days from planting
    # Removes the age effect so two fields at different growth stages can be compared fairly
    insey = ndvi_fp / days

    # Step 2: Yield potential WITHOUT nitrogen using INSEY power function
    # Coefficient 5948.45 and exponent 0.72 calibrated from Louisiana/Mississippi field data
    # Result in bu/ac; capped at max_yield
    yp0 = min(5948.45 * (insey ** 0.72), max_yield)

    # Step 3: Grain N uptake at YP0 (no added N)
...
if crop == "sugarcane":
    ...
    # Six equal columns — one per input field
    ...
    with i5:
        # Fraction of the stalk weight that is dry matter (the rest is water)
        # Sugarcane stalks are 70-75% water; N is only present in the dry fraction
        # This corrects the GNUP calculation to use dry weight instead of fresh weight
        sc_dm = st.number_input(
            "Dry Matter Fraction",
            min_value=0.10, max_value=1.0, value=0.30, step=0.01, format="%.2f",
            key="sc_dm", help="Sugarcane is 70-75% water. Dry matter = 25-30%. Default 0.30"
        )
    ...
    # ── RUN THE ALGORITHM ─────────────────────────────────────────────────────
    # Pass all 6 inputs to the function; unpack the 6 returned values
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Six equal columns — rice has one extra input (Days) compared to sugarcane
    i1, i2, i3, i4, i5, i6 = st.columns(6)
    ...
    with i2:
        ...
        rc_fp = st.number_input(
            "NDVI — Farmer's Practice",
            min_value=0.01, max_value=1.0, value=0.77, step=0.01, format="%.2f",
            key="rc_fp"
        )

    with i3:
        # Days from planting to sensing — UNIQUE to rice, needed to compute INSEY
        # Older rice plants have higher NDVI naturally; dividing by days removes that effect
        rc_days = st.number_input(
            "Days — Planting to Sensing",
            min_value=1, max_value=180, value=71,
            key="rc_days", help="Days from planting date to NDVI sensing date"
        )
    ...
    # ── RUN THE ALGORITHM ─────────────────────────────────────────────────────
    # Pass all 6 inputs to the rice function (days is the extra one vs sugarcane)
    yp0, gnup_yp0, ri, ypn, gnup_ypn, fnr = rice_calc(
        rc_fp,      # NDVI farmer's practice
        rc_nrs,     # NDVI N-Rich strip
        rc_days,    # days from planting (for INSEY)
    ...
    with right:
        # Dark blue recommendation box — INSEY is shown first in the formula since
        # that is the key step that distinguishes rice from sugarcane
        st.markdown(f"""
